...
1.  在工具箱**中**，单击 **"WebView2"Windows"窗体控件"** 以展开选项。
...
1.  在"**Windows设计器**"中，双击`Go!`按钮以在`goButton_Click`文件中创建`Form1.cs`方法。  复制并粘贴该方法中的以下代码段。
...
1.  在 文件中 `Form1.cs` ，更新构造函数以匹配以下代码段，并添加 `EnsureHttps` 函数，如下所示。
...
1.  在 文件中 `Form1.cs` ，更新构造函数并创建一个函数 `InitializeAsync` 以匹配以下代码段：
...
1.  初始化 `CoreWebView2` 后，注册事件处理程序以响应 `WebMessageReceived`。  在 文件中 `Form1.cs` ，使用 `InitializeAsync` 下面的代码 `UpdateAddressBar` 段更新和添加。
...
1. 在 文件中 `Form1.cs` ，更新以 `InitializeAsync` 匹配以下代码段：

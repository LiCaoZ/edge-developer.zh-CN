...
ms.date: 11/05/2021
ms.openlocfilehash: 8be2f9f9d110c04ee5916a377fa9ae51d4c0c5f0
ms.sourcegitcommit: ae41e2c0ca42fb7eac73824c828305c7b13b4203
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 02/09/2022
ms.locfileid: "12345938"
---
# <a name="get-started-with-webview2-in-winforms-apps"></a>WinForms 应用中的 WebView2 入门

本文将开始在 WinForms 中创建第一个 WebView2 应用，并了解 [WebView2 的主要功能](https://developer.microsoft.com/microsoft-edge/webview2)。 有关各个 API 详细信息，请参阅 [API 参考](/dotnet/api/microsoft.web.webview2.winforms)。
...
1. 选择**C# Windows Forms App (.NET Framework) **"，然后单击"下一步 **"**。
...
1. 输入名称Project**位置****的值**。  在"**框架**"下拉列表中，.NET Framework **4.7.2** 或更高版本。
...
1. 在"**添加新项**"面板中，单击"Visual **C# ItemsWeb** > **** >  **Windows FormsForm** >  ** (Windows Forms) "**，然后单击"添加 **"**。
...
1. 在工具箱**中**，单击"**WebView2 Windows窗体控件"** 展开选项。
   ...
   > [!NOTE]
   > 如果使用的是 Visual Studio 2017，默认情况下 **WebView2** 不会显示在工具箱**中**。  若要使 **WebView2** 显示在工具箱中，请选择****"**工具** > **"** > "选项"">"自动填充**工具箱**"设置设置为 `True`。****
...
1. **Source 属性**设置 WebView2 控件中显示的初始 URI。 将 **Source 属性** 设置为 `https://www.microsoft.com`。
...
1. 确保 WebView2 控件显示 [https://www.microsoft.com](https://www.microsoft.com)。
   ...
   :::image type="content" source="./media/winforms-hello-webview.png" alt-text="示例应用显示 Microsoft 网站。" lightbox="./media/winforms-hello-webview.png":::
...
1. 在 `Form1.cs`中，添加 `CoreWebView2` 命名空间，方法为在顶部插入以下代码：

   ```csharp
   using Microsoft.Web.WebView2.Core;
   ```

1. 在"**Windows设计器**"中，双击`Go!`按钮以在`goButton_Click`文件中创建 `Form1.cs` 方法。  将以下代码段粘贴到该方法中：
...
1. 在地址栏中输入新 URL，然后选择"转到 **"**。  例如，输入 `https://www.bing.com`。  确保 WebView2 控件导航到 URL。
   ...
   在地址栏中输入完整 URL。  如果 URL 不以 或 为起始，则会引发 `http://` 。`ArgumentException` `https://`
...
1. 在 文件中 `Form1.cs` ，更新构造函数以匹配以下代码，并添加 `EnsureHttps` 函数，如下所示：
...
1. 例如，添加在用户导航到非 HTTPS 网站时发送警报的脚本。  修改 函数 `EnsureHttps` 以将脚本注入到使用 [ExecuteScriptAsync](/dotnet/api/microsoft.web.webview2.winforms.webview2.executescriptasync) 方法的 Web 内容中，如下所示：
...
1. 在 文件中 `Form1.cs` ，更新构造函数并创建一个函数 `InitializeAsync` 以匹配以下代码：
...
1. 初始化 `CoreWebView2` 后，注册事件处理程序以响应 `WebMessageReceived`。  在 文件中 `Form1.cs` ，使用 `InitializeAsync` 下面的代码 `UpdateAddressBar` 更新和添加：
...
1. 对于 WebView2 发送和响应 Web 消息 `CoreWebView2` ，在初始化后，主机将 Web 内容中的脚本注入到：
   1. 使用 将 URL 发送到主机 `postMessage`。
   1. 注册事件处理程序以打印从主机发送的消息。

1. 在 文件中 `Form1.cs` ，更新以 `InitializeAsync` 匹配以下代码：
...
1. 打开新的 URI 时，WebView2 控件会在地址栏中显示它。
   ...
   应用程序在地址栏中显示 URI `https://www.microsoft.com` 。  Microsoft 网站将显示在窗口中。

   :::image type="content" source="./media/winforms-final-app.png" alt-text="应用程序在地址栏中显示 URI。":::

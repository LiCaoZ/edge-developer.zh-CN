...
有关 SDK 版本如何与 WebView2 运行时或预览频道结合使用的详细信息，Microsoft Edge了解[WebView2 SDK 版本][WebView2ConceptsVersioning]。
...
1.  设置为 `CreationProperties.BrowserExecutableFolder` 指向包含 Canary 或 Dev Microsoft Edge的路径。  为此，在 **WebView2Samples** 解决方案中的 **WebView2WpfBrowser** 项目中，打开文件 `MainWindow.xaml.cs` 。
...
若要使应用程序使用设置浏览器Microsoft Edge文件夹的注册表替代来使用预览通道：

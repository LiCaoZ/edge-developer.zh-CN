...
ms.openlocfilehash: 80b0b55ac64311212777e8d2bbd0be7fae08fbc9
ms.sourcegitcommit: 148b9b2f609eb775ed7fd71d50ac98a829ca90df
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 11/02/2021
ms.locfileid: "12141850"
---
...
> [!NOTE]
> 请确保在更新 WebView2 SDK NuGet WebView2 应用。  WebView2 团队建议执行以下操作：
...
>
> 有关详细信息，请参阅 [将运行时版本与 SDK 版本匹配][Webview2ConceptsVersioningMatchingWebview2RuntimeVersions]。
...
若要将预发行 SDK 与 Microsoft Edge预览频道一同使用，请导航到切换到预览频道以测试即将推出的[API 和功能][SwitchToPreviewChannel]。
...
[NuGet WebView2 SDK 1.0.992.28 的程序包][NuGetGallery1.0.992.28]
...
*   修复了缺少的 WebView2 (未在用户的 .NET 项目中) 导致初始化失败 `PlatformTarget` 。  ([问题 #1061][GithubMicrosoftedgeWebviewfeedbackIssue1061]) 

#### <a name="promotions"></a>促销

以下项目现已稳定：
*   [OpenTaskManagerWindow API][OpenTaskManagerWindowAPI]。
*   [isSwipeNavigationEnabled 属性][isSwipeNavigationEnabledproperty]。
*   [BrowserProcessExited API][BrowserProcessExitedAPI]。
*   [get_Name接口上的][get_Nameproperty] 属性 `ICoreWebView2NewWindowRequestedEventArgs2` 。
...
[NuGet WebView2 SDK 1.0.1018 预发行包][NuGetGallery1.0.1018-prerelease]
...
#### <a name="experimental-features"></a>实验性功能
*   添加了 [媒体 API，][Webview2ReferenceWin32Icorewebview2experimental9ViewWebview2101018Prerelease] 使开发人员能够将 WebView 中的媒体静音/取消静音。
*   添加了对使用 WebView2 [的多个][Webview2ReferenceWin32Icorewebview2experimentalenvironment8ViewWebview2101018Prerelease] 用户配置文件的支持。

#### <a name="bug-fixes"></a>Bug 修复
*   修复了在应用跨越监视器和监视器缩放更改时 WebView2 停止呈现的 Bug。
*   修复了在多个下载窗口打开时关闭下载 UI 崩溃 WebView2 的问题。  (问题 [#1723][GithubMicrosoftedgeWebviewfeedbackIssue1723]) 
*   修复了在用户的 .NET 项目中未设置 PlatformTarget 时生成/初始化错误。  ([问题 730][GithubMicrosoftedgeWebviewfeedbackIssue730] 和问题 [#1548][GithubMicrosoftedgeWebviewfeedbackIssue1548]) 
...
[NuGet WebView2 SDK 1.0.1010 预发行包][NuGetGallery1.0.1010-prerelease]

为了完全实现 API 兼容性，此预发布版本的 WebView2 SDK Microsoft Edge版本 95.0.1010.0 或更高版本。

### <a name="general"></a>概要
*   WebView2 性能改进。
*   可靠性修补程序。  ([问题 #1605][GithubMicrosoftedgeWebviewfeedbackIssue1605] 和 [问题 #1678][GithubMicrosoftedgeWebviewfeedbackIssue1678]) 
*   在启动期间和主机应用位于前台时添加了性能改进。

#### <a name="experimental-features"></a>实验性功能
*   使用 删除无提示失败，当调用时，将引发 具有 `EnsureCoreWebView2Async` `ArgumentException` 不兼容参数的多个错误。
*   更改了环境 [对象中 UserDataFolder][Webview2ReferenceWin32Icorewebview2experimentalenvironment5ViewWebview210999PrereleaseGetUserdatafolder] 属性的默认处理。
    > [!CAUTION]
    > **中断更改**：如果开发人员未指定要在何处进行设置，UDF 的默认处理将更改。 有关详细信息，请导航到["通知：用户目录文件夹默认处理更新"。][WebView2UDFAnnouncement]
*   添加了 [导航&][Webview2ReferenceWin32Icorewebview2experimentalframeViewWebview210999Prerelease] iframe 脚本 API。
*   添加了 [MemoryUsageTargetLevel，][Webview2ReferenceWin32ICoreWebView2Experimental5ViewWebview210999Prerelease] 允许开发人员指定内存消耗级别，如低或正常。
*   向[环境选项添加了 ExclusiveUserDataFolderAccess。][Webview2ReferenceWin32ICoreWebView2ExperimentalEnvironmentOptionsViewWebview210999Prerelease]
*   添加了 [HiddenPdfToolbarItems][Webview2ReferenceWin32ICoreWebView2ExperimentalSettings6ViewWebview210999Prerelease] 以自定义 PDF 工具栏项。
*   添加了 [PrintToPdf][Webview2ReferenceWin32ICoreWebView2ExperimentalPrintToPdfCompletedHandlerViewWebview210999Prerelease]，允许将当前页面打印为 PDF。 此外，还可以在此新 API 中使用可选的自定义设置。
*   添加了 [AllowExternalDrop][Webview2ReferenceWin32ICoreWebView2ExperimentalCompositionController3ViewWebview210999Prerelease] 属性以允许从 WebView2 控件外向其中拖放对象。
*   添加了 [ContextMenu API，][Webview2ReferenceWin32ICoreWebView2ExperimentalContextMenuItemViewWebview210999Prerelease] 允许自定义 WebView2 上下文菜单。

#### <a name="bug-fixes"></a>Bug 修复
*   改进了在 JavaScript 代码中捕获主机对象异常的方法。
*   使用 DevTools 窗口中的通用图标替换了 WebView2 图标。
*   使用时打开选项卡屏幕 `MediaDevices.getDisplayMedia()` 共享选项。  ([问题 #1566][GithubMicrosoftedgeWebviewfeedbackIssue1566]) 
*   修复了未选择正确证书时客户端证书 API 中的 Bug。 这是运行时更改。  (问题 [#1666][GithubMicrosoftedgeWebviewfeedbackIssue1666]) 
*   修复了 `window.chrome.webview` 在同一父域中的新窗口中不可用的错误。 此更改特定于运行时。  ([问题 #1144][GithubMicrosoftedgeWebviewfeedbackIssue1144]) 
*   修复了下拉菜单或列表显示在具有焦点的窗口后面的 Bug。  ([问题 #411][GithubMicrosoftedgeWebviewfeedbackIssue411]) 
*   修复了使用 时的焦点问题 `put_IsVisible(false)` 。  ([问题 #238][GithubMicrosoftedgeWebviewfeedbackIssue238]) 
...
[NuGet WebView2 SDK 1.0.961.33 的程序包][NuGetGallery1.0.961.33]
...
*   修复了 WebView2 .NET 控件中的一个 bug，即在后台创建 WebView2 窗口时，这些窗口为空。  (问题 [#1077][GithubMicrosoftedgeWebviewfeedbackIssue1077]) 
*   修复了用户导航到新窗口或显示页面时未更新设置 `about:blank` 的问题。 这是运行时更改。

#### <a name="promotions"></a>促销

以下项目现已稳定：
*   [客户端证书 API][Webview2ReferenceWin32Icorewebview25ViewWebview21096132AddClientcertificaterequested]。
...
[NuGet WebView2 SDK 1.0.955 预发行包][NuGetGallery1.0.955-prerelease]
...
#### <a name="experimental-features"></a>实验性功能
*   添加了 [OpenTaskManagerWindow][Webview2ReferenceWin32Icorewebview2experimental4ViewWebview210955PrereleaseOpentaskmanagerwindow] 以启动 WebView2 浏览器任务管理器。
*   添加了 [NewWindowRequestedEventArgs][Webview2ReferenceWin32Icorewebview2experimentalnewwindowrequestedeventargsViewWebview210955PrereleaseGetName]。
*   添加了对虚拟主机名映射的支持，以使用服务工作人员。
*   添加了 [HiddenPdfToolbarItems][Webview2ReferenceWin32Icorewebview2experimentalsettings6ViewWebview210955PrereleaseGetHiddenpdftoolbaritems] 以自定义 PDF 工具栏项。
...
*   修复了 `add_WebResourceRequested` 在使用 创建的 WebView2 控件上不起作用时的错误 `add_NewWindowRequested` 。  ([问题 #616][GithubMicrosoftedgeWebviewfeedbackIssue616]) 
*   允许主机应用在不同的应用程序上设置前台，以响应事件，包括 、 `NavigationStarting` `AddHostObjectToScript` 方法和 `WebMessageReceived` `NewWindowRequested` 。  ([问题 #1092][GithubMicrosoftedgeWebviewfeedbackIssue1092]) 
*   修复了用于触发 `PermissionRequested` 麦克风事件的错误。 此更改特定于运行时。 ([问题 #1462][GithubMicrosoftedgeWebviewfeedbackIssue1462]) 
*   修复了多次 `ExecuteScriptAsync` 成功运行后被阻止的错误。 此更改特定于运行时。  (问题 [#1348][GithubMicrosoftedgeWebviewfeedbackIssue1348]) 
*   修复了防止 在 中使用非 ASCII 文件名 `ResultFilePath` 的错误 `DownloadStartingEventArgs` 。  (问题 [#1428][GithubMicrosoftedgeWebviewfeedbackIssue1428]) 
*   修复了默认弹出窗口上的标题栏未完全显示的问题。 此更改特定于运行时。  ([问题 1016][GithubMicrosoftedgeWebviewfeedbackIssue1016]) 

#### <a name="promotions"></a>促销
*   [add_ClientCertificateRequested][Webview2ReferenceWin32Icorewebview25ViewWebview210955PrereleaseAddClientcertificaterequested] 已提升为稳定。
...
*   修复了在包含子表单的表单上 WebView2 崩溃的 Bug。 在页面栏中打开查找的子窗体导致 WebView2 在关闭子窗体时崩溃。  ([问题 #1097][GithubMicrosoftedgeWebviewfeedbackIssue1097]) 
...
[NuGet WebView2 SDK 1.0.902.49 的程序包][NuGetGallery1.0.902.49]
...
#### <a name="bug-fixes"></a>Bug 修复
*   修复了导致属性出错的 bug，它已关闭在出现导航失败或呈现进程失败时 `IsBuiltInErrorPageEnabled` 显示的错误页面。  此更改特定于运行时。  ([问题 #634][GithubMicrosoftedgeWebviewfeedbackIssue634]) 
*   修复了 WebView2 控件焦点离开用户焦点的问题。
*   修复了 `AddScriptToExecuteOnDocumentCreated` 在子窗口无法工作时的错误。  (问题 [#935][GithubMicrosoftedgeWebviewfeedbackIssue935]) 
*   修复了导致非活动选项卡被自动丢弃的 Bug。  ([问题 #637][GithubMicrosoftedgeWebviewfeedbackIssue637]) 
*   修复了导航事件被另一个导航事件中断导致事件的导航 ID `NavigationCompleted` 不正确时的错误。  ([问题 #1142][GithubMicrosoftedgeWebviewfeedbackIssue1142]) 
...
*   [下载 API][Webview2ReferenceWin32Icorewebview24ViewWebview210902PrereleaseAddDownloadstarting]。
*   [AddHostObjectToScriptWithOrigins][AddHostObjectToScriptWithOriginsAPI] 支持 iFrame 元素的 API。
...
[NuGet WebView2 SDK 1.0.902 预发行包][NuGetGallery1.0.902-prerelease]
...
*   添加了 [add_ClientCertificateRequested API][Webview2ReferenceWin32Icorewebview2experimental3ViewWebview210902PrereleaseAddClientcertificaterequested]。 它允许显示客户端证书对话框提示（如果需要）并允许访问所需的元数据以替换默认客户端证书对话框提示。
...
*   修复了 WebView2 .NET 控件中的 Bug，其中 WebView2 窗口在后台创建时为空。  ([问题 #1077][GithubMicrosoftedgeWebviewfeedbackIssue1077]) 。
*   通过按 WebView 控件使 WPF 应用程序崩溃或不再崩溃，关闭 `Enter` `Esc` 文件选取器对话框。  ([问题 #1099][GithubMicrosoftedgeWebviewfeedbackIssue1099]) 。
*   修复了在附加事件处理程序时 [AllowSingleSignOnUsingOSPrimaryAccount][Webview2ReferenceWin32Icorewebview2environmentoptionsGetAllowsinglesignonusingosprimaryaccount] 无法正常使用 WebView2 `WebResourceRequested` 的问题。 此更改特定于运行时。  ([问题 #1183][GithubMicrosoftedgeWebviewfeedbackIssue1183]) 。
*   下载文件不再会破坏 WebView2 `DefaultBackgroundColor` 的透明度。 此更改特定于运行时。  ([问题 #1108][GithubMicrosoftedgeWebviewfeedbackIssue1108]) 。
*   删除了包含 Microsoft 品牌打造的屏幕共享媒体选取器消息。  (问题 [940][GithubMicrosoftedgeWebviewfeedbackIssue940]) 。
*   修复了 WebView2 WinForm 控件中隐藏父表单不会隐藏 WebView2 控件的 bug ([Issue #828][GithubMicrosoftedgeWebviewfeedbackIssue828] and [Issue #1079][GithubMicrosoftedgeWebviewfeedbackIssue1079]) 。
*   向 WebView2 WS_CLIPCHILDREN添加了静态样式样式。  ([问题 #1013][GithubMicrosoftedgeWebviewfeedbackIssue1013]) 。
...
*   [下载 API][Webview2ReferenceWin32Icorewebview24ViewWebview210902PrereleaseAddDownloadstarting] 现已升级为稳定。
...
*   修复了 WebView2 WinForm 控件中释放父窗口后 WebView2 窗口可见性未正确更新的 Bug。  ([问题 #1282][GithubMicrosoftedgeWebviewfeedbackIssue1282] 和 [问题 #828][GithubMicrosoftedgeWebviewfeedbackIssue828]) 。
*   修复了 WebView2 WPF 控件中的一个错误，即 WPF OneWay 绑定模式下的 Source 属性绑定无法正常工作。  ([问题 619][GithubMicrosoftedgeWebviewfeedbackIssue619] 和问题 [#608][GithubMicrosoftedgeWebviewfeedbackIssue608]) 。
...
[NuGet WebView2 SDK 1.0.864.35 的程序包][NuGetGallery1.0.864.35]
...
*   修复了项目找不到文件的 818 SDK 发布包中的 `WebView2.h` 错误。  ([问题 #1209][GithubMicrosoftedgeWebviewfeedbackIssue1209]) 。
*   修复了一个缺陷，该 Bug 导致对具有二进制主体的一些请求丢弃 WebResourceRequested 事件。
*   改进 `NewWindowRequested` 文档。  ([问题 #448][GithubMicrosoftedgeWebviewfeedbackIssue448]) 。
...
#### <a name="bug-fixes"></a>Bug 修复
*   修复了 WebView2 .NET 控件中在访问标头集合时缺少第一 `CoreWebView2WebResourceRequest` 个标头的 bug。  (问题 [#1123][GithubMicrosoftedgeWebviewfeedbackIssue1123]) 。
...
[NuGet WebView2 SDK 1.0.865 预发行包][NuGetGallery1.0.865-prerelease]
...
*   关闭下载 `OpenSaveAsAwareness` UI，因为它包含指向 的链接 `edge://settings` 。   ([问题 #1120][GithubMicrosoftedgeWebviewfeedbackIssue1120]) 。
*   从屏幕共享对话框中删除了品牌。   (问题 [940][GithubMicrosoftedgeWebviewfeedbackIssue940]) 。
*   修复了当 [SetWindowDisplayAffinity][WindowsWin32ApiWinuserSetWindowDisplayAffinity] 函数在 WebView2 应用中停止屏幕捕获时使 WebView2 成为错误。   ([问题 #841][GithubMicrosoftedgeWebviewfeedbackIssue841]) 。
...
*   向 WPF 工具箱添加了 WebView2 设计器工具。   ([问题 #210][GithubMicrosoftedgeWebviewfeedbackIssue210]) 。
*   在 .NET Designer 模式下添加了 WebView2 UI 元素。

#### <a name="bug-fixes"></a>Bug 修复

*   改进了 COM 异常描述，将每个异常包装在更详细的 .NET 异常中。   (问题 [#338][GithubMicrosoftedgeWebviewfeedbackIssue338]) 。  此更改特定于运行时。
*   修复了在选择切换焦点时导致 WebView2 控件在 Microsoft Visual Studio Tools for Office 中 `Tab` 崩溃Office。   (问题 [#589][GithubMicrosoftedgeWebviewfeedbackIssue589] 和 [问题 #933][GithubMicrosoftedgeWebviewfeedbackIssue933]) 。  此更改特定于运行时。
*   改进了 .NET framework 加载程序下层级别，更可靠。   ([问题 #946][GithubMicrosoftedgeWebviewfeedbackIssue946]) 。
*   修复了在首次导航完成之前尝试刷新时导致崩溃的 Bug。   ([问题 #1011][GithubMicrosoftedgeWebviewfeedbackIssue1011]) 。
*   修复了初始化，因此导航在 期间发生 `CoreWebView2InitializationCompleted` 。   ([问题 #1050][GithubMicrosoftedgeWebviewfeedbackIssue1050]) 。
*   改进了 .NET 浏览器进程崩溃错误处理。  现在可以在未发生崩溃的情况下处理 `ProcessFailed` 事件后重新创建控件。   (问题 [996][GithubMicrosoftedgeWebviewfeedbackIssue996]) 。
...
[NuGet WebView2 SDK 1.0.818.41 的程序包][NuGetGallery1.0.818.41]
...
*   改进的 WebView2 代码可以更弹性地使用格式错误的版本信息 `.exe` 处理应用程序文件。   ([问题 #850][GithubMicrosoftedgeWebviewfeedbackIssue850]) 。
*   从 `--winhttp-proxy-resolver` WebView 浏览器进程命令行中删除，为 WebView2 打开其他代理命令行选项。
...
[NuGet WebView2 SDK 1.0.824 预发行包][NuGetGallery1.0.824-prerelease]
...
*   Rasterization Scale API ([RasterizationScale][Webview2ReferenceWin32Icorewebview2experimentalcontrollerViewWebview210721PrereleaseGetRasterizationscale] 属性  [、RasterizationScaleChanged][Webview2ReferenceWin32Icorewebview2experimentalcontrollerViewWebview210721PrereleaseAddRasterizationscalechanged] 事件 [、BoundsMode][Webview2ReferenceWin32Icorewebview2experimentalcontrollerViewWebview210721PrereleaseGetBoundsmode]属性和 [ShouldDetectMonitorScaleChanges][Webview2ReferenceWin32Icorewebview2experimentalcontrollerViewWebview210721PrereleaseGetShoulddetectmonitorscalechanges] 属性) 现在提升为 Stable。

#### <a name="bug-fixes"></a>Bug 修复

*   扩展支持的 C++ 和 .NET 项目类型，如 MFC 和 ATL。   ([问题 506、][GithubMicrosoftedgeWebviewfeedbackIssue506][问题 669][GithubMicrosoftedgeWebviewfeedbackIssue669]和[问题 #851][GithubMicrosoftedgeWebviewfeedbackIssue851]) 。
*   修复了 Evergreen WebView2 运行时泄露入站防火墙条目的 Bug。
*   修复了事件期间的响应 `WebResourceRequested` 设置。   (问题 [568][GithubMicrosoftedgeWebviewfeedbackIssue568]) 。
*   修复了导航到导致 `edge://` 浏览器进程退出的 Bug。   ([问题 #604][GithubMicrosoftedgeWebviewfeedbackIssue604]) 。
*   修复了在可视化托管模式下将 WebView2 边界限制为屏幕大小的 Bug。
...
[NuGet WebView2 SDK 1.0.774.44 的程序包][NuGetGallery1.0.774.44]

为了完全兼容 API，此版本的 WebView2 SDK 需要 WebView2 运行时版本 89.0.774.44 或更高版本。
...
[NuGet WebView2 SDK 1.0.790 预发行包][NuGetGallery1.0.790-prerelease]
...
*   添加了 [SetVirtualHostNameToFolderMapping][Webview2ReferenceWin32Icorewebview23ViewWebview210790PrereleaseSetvirtualhostnametofoldermapping] 方法，该方法将虚拟主机名映射到目录路径。   ([问题 37、][GithubMicrosoftedgeWebviewfeedbackIssue37][问题 #161][GithubMicrosoftedgeWebviewfeedbackIssue161]和问题[#212][GithubMicrosoftedgeWebviewfeedbackIssue212]) 。
*   添加了 [DefaultBackgroundColor][Webview2ReferenceWin32Icorewebview2controller2ViewWebview210790PrereleaseGetDefaultbackgroundcolor] 属性以设置背景的颜色和 alpha 通道。   ([问题 #414][GithubMicrosoftedgeWebviewfeedbackIssue414]) 。
*   添加了 [UserAgent][Webview2ReferenceWin32Icorewebview2experimentalsettingsViewWebview210790PrereleaseGetUseragent] 属性以获取或设置用户代理。   (问题 [122][GithubMicrosoftedgeWebviewfeedbackIssue122]) 。
...
*   在 为 时关闭 PDF 查看器中的上下文 `AreDefaultContextMenusEnabled` 菜单 `false` 。   (问题 [605][GithubMicrosoftedgeWebviewfeedbackIssue605]) 。
*   修复了在查询 时 `E_NOINTERFACE` 返回的 `ICoreWebView2` `ICoreWebView2Experimental` Bug。   (问题 [691][GithubMicrosoftedgeWebviewfeedbackIssue691]) 。
*   修复了将 设置为 时允许使用格式错误的 URI `CoreWebView2NavigationStartingEventArgs.Cancel` 进行导航的 `false` bug。   ([问题 #400][GithubMicrosoftedgeWebviewfeedbackIssue400]) 。
*   修复了在弹出窗口中阻止的 Bug，并 `window.print()` 附带事件 `NewWindowRequested` 的事件处理程序。   ([问题 #409][GithubMicrosoftedgeWebviewfeedbackIssue409]) 。
*   修复了在不同监视器之间移动应用时的动态 DPI 问题。   ([问题 58][GithubMicrosoftedgeWebviewfeedbackIssue58]) 
...
*   关闭自动填充管理按钮。   ([问题 585][GithubMicrosoftedgeWebviewfeedbackIssue585]) 。
*   修复Visual Studio窗口托管时运行时 `WebView2.Dispose` 崩溃的问题。   ([问题 #816) ][GithubMicrosoftedgeWebviewfeedbackIssue816][和问题 #442][GithubMicrosoftedgeWebviewfeedbackIssue442]) 。
*   修复了在工具箱中显示 WebView2 Visual Studio Bug。   ([问题 210][GithubMicrosoftedgeWebviewfeedbackIssue210]) 。
*   减少了高 CPU 使用率问题。   ([问题 #878][GithubMicrosoftedgeWebviewfeedbackIssue878]) 。
*   修复了已弃用 1.0.781-prerelease 程序包的问题。   ([问题 #875][GithubMicrosoftedgeWebviewfeedbackIssue875] 和 [问题 #878][GithubMicrosoftedgeWebviewfeedbackIssue878]) 。
...
*   修复了使用 WPF SDK 的 WebView 应用崩溃的错误。  当你选择关闭窗口时 `F4` 发生崩溃。   ([问题 #399][GithubMicrosoftedgeWebviewfeedbackIssue399]) 。
*   WebView2 初始化屏幕现在是透明的，而不是灰色的。   ([问题 #196][GithubMicrosoftedgeWebviewfeedbackIssue196]) 。
...
[NuGet WebView2 SDK 1.0.705.50 的程序包][NuGetGallery1.0.705.50]
...
[NuGet WebView2 SDK 1.0.721 预发行包][NuGetGallery1.0.721-prerelease]

此预发布版本的 WebView2 SDK Microsoft Edge版本 86.0.616.0 或更高版本。

### <a name="general"></a>概要

> [!IMPORTANT]
> **中断更改**：已弃用 WebView2 预发布包 1.0.707 和程序包 0.9.628。  停止使用程序包 1.0.707 和 package0.9.628 进行开发。
...
*   更新 `NewWindowRequested` 了处理 和 `Ctrl` + `click` 的事件 `Shift` + `click` 。   ([问题 #168][GithubMicrosoftedgeWebviewfeedbackIssue168] 和 [问题 #371][GithubMicrosoftedgeWebviewfeedbackIssue371]) 。
*   以下实验 API 现已提升为稳定。
    *   [WebResourceResponseReceived API][Webview2ReferenceWin32Icorewebview22ViewWebview210721PrereleaseAddWebresourceresponsereceived]
    *   [NavigateWithWebResourceRequest API][Webview2ReferenceWin32Icorewebview2environment2ViewWebview210721PrereleaseCreatewebresourcerequest]
    *   [Cookie 管理 API][Webview2ReferenceWin32Icorewebview2cookiemanagerViewWebview210721Prerelease]
...
*   改进了 .NET Cookie 管理。   ([问题 #611][GithubMicrosoftedgeWebviewfeedbackIssue611]) 。
...
*   添加了 [AcceleratorKeyPressed][DotnetApiMicrosoftWebWebview2WpfWebview2Acceleratorkeypressed] 事件以支持 `AcceleratorKey` 在 WebView2 中选择。   ([问题 #288][GithubMicrosoftedgeWebviewfeedbackIssue288]) 。
*   删除了输出到 WebView2 文件夹的不必要的文件。   ([问题 #461][GithubMicrosoftedgeWebviewfeedbackIssue461]) 。
*   改进了主机对象 API。   ([问题 335][GithubMicrosoftedgeWebviewfeedbackIssue335] 和问题 [525][GithubMicrosoftedgeWebviewfeedbackIssue525]) 。
...
[NuGet WebView2 SDK 1.0.664.37 的程序包][NuGetGallery1.0.664.37]
...
*   `CoreWebView2NewWindowRequestedEventArgs.Handled` 阻止打开新窗口。   ([问题 549][GithubMicrosoftedgeWebviewfeedbackIssue549] 和问题 [560][GithubMicrosoftedgeWebviewfeedbackIssue560]) 。
...
[NuGet WebView2 SDK 1.0.674 预发行包][NuGetGallery1.0.674-prerelease]
...
*   添加了 [允许开发人员][Webview2ReferenceWin32Icorewebview2experimentalViewWebview210674PrereleaseGetCookiemanager] 对 WebView2 会话进行身份验证或从 WebView 检索 Cookie 以验证其他工具的 Cookie 管理 API。  WebView2 团队计划进行特定于语言或框架的改进。  有关详细信息，请导航到["API 审阅：Cookie 管理"。][GithubMicrosoftedgeWebview2AnnouncementIssue2]
...
*   将命名空间中自定义 EventArgs 类的实现更改为 `Microsoft.Web.WebView2.Core` [System.EventArgs][DotnetApiSystemEventargs] 或 [System.ComponentModel.CancelEventArgs 的子类][DotnetApiSystemComponentmodelCancelEventargs]。   ([问题 #250][GithubMicrosoftedgeWebviewfeedbackIssue250]) 
*   增加了对 [WinForms 中 CoreWebView2CreationProperties][DotnetApiMicrosoftWebWebview2Winforms] 的支持。   ([问题 #204][GithubMicrosoftedgeWebviewfeedbackIssue204]) 。
*   添加了 [WebResourceRequested][DotnetApiMicrosoftWebWebview2CoreCorewebview2Webresourcerequested] .NET API。   ([问题 #219][GithubMicrosoftedgeWebviewfeedbackIssue219]) 。
*   将 WinForms Designer [Source][DotnetApiMicrosoftWebWebview2WinformsWebview2Source] 属性更新为默认值或重置为 null。   (问题 [#177][GithubMicrosoftedgeWebviewfeedbackIssue177]) 。
*   更新了 WebView2.Init () 中的 WebView2 边界，以支持小于 100% 的 DPI 模式。   ([问题 #432][GithubMicrosoftedgeWebviewfeedbackIssue432]) 。
*   更新 [了 BuildWindowCore][DotnetApiMicrosoftWebWebview2WpfWebview2Buildwindowcore] 和 [DestroyWindowCore][DotnetApiMicrosoftWebWebview2WpfWebview2Destroywindowcore] 以提高稳定性。   ([问题 #382][GithubMicrosoftedgeWebviewfeedbackIssue382]) 。
*   更新了 .NET 加载程序基，以在进程位（而不是操作系统体系结构）上加载。   (问题 [#431][GithubMicrosoftedgeWebviewfeedbackIssue431]) 。
...
[NuGet WebView2 SDK 1.0.622.22 的程序包][NuGetGallery1.0.622.22]
...
*   Evergreen WebView2 运行时和安装程序是 GA。  引导程序、引导程序下行链接和 Evergreen WebView2 运行时的独立安装程序Microsoft Edge [WebView2。][MicrosoftDeveloperMicrosoftEdgeWebView2]  [WebView2Samples][GithubMicrosoftedgeWebview2samplesMain]存储库也提供了安装工作流的示例代码。
*   固定版本模式适用于开发人员预览。

有关运行时、常青分发和固定版本分发的信息，请导航到分发 [WebView2 应用和 WebView2 运行时][Webview2ConceptsDistribution]。
...
[NuGet WebView2 SDK 0.9.622.11 的程序包][NuGetGallery0.9.622.11]
...
*   进行了 `ICoreWebView2NewWindowRequestedEventArgs` 更新以包括 [WindowFeatures][Webview2ReferenceWin32Icorewebview2newwindowrequestedeventargsViewWebview209622GetWindowfeatures] 属性和关联的 [ICoreWebView2WindowFeatures][Webview2ReferenceWin32Icorewebview2windowfeaturesViewWebview209622]。   (问题 [293][GithubMicrosoftedgeWebviewfeedbackIssue293]) 。
*   进行了 `System.Windows.Rect`  更新以 `System.Drawing.Rectangle` 使用而不是 `System.Windows.Rect` (问题 [#235][GithubMicrosoftedgeWebviewfeedbackIssue235]) 。
*   更新了 NewWindowRequested 事件以处理 `window.open()` 不带参数的请求。   (问题 [293][GithubMicrosoftedgeWebviewfeedbackIssue293]) 。
...
[NuGet WebView2 SDK 0.9.579 的程序包][NuGetGallery0.9.579]
...
*   > [!IMPORTANT]
    > **公告**：Evergreen WebView2 运行时和安装程序发布了预览版。  有关详细信息，请导航到分发 [WebView2 应用和 WebView2 运行时][Webview2ConceptsDistribution]。
...
*   添加了 WebView 工作线程改进。   ([问题 #318][GithubMicrosoftedgeWebviewfeedbackIssue318]) 。
...
*   改进了 WPF 焦点处理。   ([问题 #185][GithubMicrosoftedgeWebviewfeedbackIssue185]) 。
...
[NuGet WebView2 SDK 0.9.538 的程序包][NuGetGallery0.9.538]

此版本的 WebView2 SDK Microsoft Edge版本 85.0.538.0 或更高版本。

### <a name="general"></a>概要

*   放弃对 WebView2 SDK 版本 [0.8.149 的支持](#08149)。  WebView2 建议使用最新版本的 WebView2 保持最新。
*   更新了组策略，以考虑Microsoft Edge浏览器的配置文件路径 ([#179][GithubMicrosoftedgeWebviewfeedbackIssue179]) 。
...
*   更新 [了 AddHostObjectToScript][Webview2ReferenceWin32Icorewebview2ViewWebview209538ddhostobjecttoscript]。  原来的主机对象序列化器标记现在设置为代理对象。  然后，当在 JavaScript 回调 ([#148][GithubMicrosoftedgeWebviewfeedbackIssue148]) 中作为参数传递时，主机对象序列化标记将序列化回主机) 。
...
*   添加了对可视化托管和窗口功能实验 [API 的支持][Webview2ConceptsVersioningExperimentalApis]。
*   > [!IMPORTANT]
...
[NuGet WebView2 SDK 0.9.515 预发行包][NuGetGallery0.9.515-prerelease]
...
*   有关生成 Windows Forms 应用的信息，请导航到[WinForms][Webview2GetStartedWinforms]应用中的 WebView2 入门和 Windows [Forms][DotnetApiMicrosoftWebWebview2Winforms]特定 API 的 WebView2 Windows Forms Reference。
...
[NuGet WebView2 SDK 0.9.488 的程序包][NuGetGallery0.9.488]

此版本的 WebView2 SDK 要求Microsoft Edge版本 84.0.488.0 或更高版本。

*   > [!IMPORTANT]
    > **公告**：从即将发布的 Microsoft Edge版本 83 开始，Evergreen WebView 不再面向稳定浏览器渠道。  相反，它面向另一组二进制文件（品牌为 Evergreen WebView2 Runtime）中，可以通过 WebView2 团队当前正在开发的安装程序进行链接安装。  有关详细信息，请导航到分发 [WebView2 应用和 WebView2 运行时][Webview2ConceptsDistribution]。

*   > [!IMPORTANT]
    > 公告 **：今后**，WebView2 团队发布两个程序包：包含实验性 API (的预发布包) 和具有稳定 API (的稳定发布包，用于增强可信度) 。  若要了解差异，请导航到了解[浏览器版本和 WebView2。][Webview2ConceptsVersioning]
...
*   添加了 [FrameNavigationCompleted][Webview2ReferenceWin32Icorewebview2ViewWebview209488AddFramenavigationcompleted] 事件。  现在，当元素完成导航时，将运行事件并返回导航 `iframe` 和导航 ID 的成功。
...
*   更新了远程对象注入以支持 (`IDispatch` [#113][GithubMicrosoftedgeWebviewfeedbackIssue113]) 。
*   更新 [了 NewWindowRequested][Webview2ReferenceWin32Icorewebview2ViewWebview209488AddNewwindowrequested] 事件以处理来自上下文菜单的请求 ([#108][GithubMicrosoftedgeWebviewfeedbackIssue108]) 。
*   发布了第一个单独的 WebView2 预发布包，你可以访问可视托管 API。  WebView2 团队更新 [了 APISample][GithubMicrosoftedgeWebview2samplesMain] 以包含新的实验性 API。
...
[NuGet WebView2 SDK 0.9.430 的程序包][NuGetGallery0.9.430]

此版本的 WebView2 SDK 要求Microsoft Edge版本 82.0.430.0 或更高版本。

WebView2 SDK 是官方 Win32 C++ Beta 版本，它包含了来自反馈的多项功能请求。  WebView2 团队尝试通过发生重大变化来限制发布数量。  随着通用方法的推出，Beta 版本中将包含一些重大重大更改。

*   > [!IMPORTANT]
    > **更改：** 在最终版本接近最终发布时，WebView2 团队将前缀重命名为 ，以确保 WebView2 API 符合 Windows `IWebView2WebView` `ICoreWebView2` API 命名约定。  此外，为了从 UI 框架利用 WebView2 SDK，WebView2 团队分为 `ICoreWebView2` [ICoreWebView2][Webview2ReferenceWin32Icorewebview2ViewWebview209430] 和 [ICoreWebView2Host][Webview2ReferenceWin32Icorewebview2hostViewWebview209430]。  `ICoreWebView2Host` 支持调整大小、显示和隐藏、对焦以及与窗口和合成相关的其他功能。  ICoreWebView2 支持所有其他 WebView2 功能。  若要了解有关合并更改的信息，请导航到 [WebView2][GithubMicrosoftedgeWebview2samplesPr17] [APISample][GithubMicrosoftedgeWebview2samplesMain] 项目中的 WebView2 拉取请求。
...
*   添加了[IsZoomControlEnabled][Webview2ReferenceWin32Icorewebview2settingsViewWebview209430GetIszoomcontrolenabled]设置，以防止用户使用 和 +鼠标滚轮 (WebView `ctrl` + `+` `ctrl` + `-` `ctrl` 的缩放) 。  当关闭该 [设置put_ZoomFactor，][Webview2ReferenceWin32Icorewebview2hostViewWebview209430PutZoomfactor] 仍可使用"缩放"来设置缩放。
...
[NuGet WebView2 SDK 0.8.355 的程序包][NuGetGallery0.8.355]
...
*   添加了对除英语 [#30][GithubMicrosoftedgeWebviewfeedbackIssue30] (之外的所有语言的 IME) 。
...
*   添加了Visual Studio和Visual Studio Code对 WebView2 的调试支持。  现在，从 IDE 在 WebView2 中调试脚本。  有关详细信息，请导航到 [如何使用 WebView2 控件进行开发时进行调试][Webview2HowToDebug]。
...
*   已 `DPI Awareness` 更新 。  现在，WebView 的 DPI 感知与主机应用的 DPI 感知相同。
    ...
    > [!NOTE]
    > 如果启动的另一个混合应用与原始 WebView 的 DPI 感知不同，则新 WebView 不会启动（如果 与 (`user data folder` [#1][GithubMicrosoftedgeWebviewfeedbackIssue1] 相同) 。
...
[NuGet WebView2 SDK 0.8.270 的程序包][NuGetGallery0.8.270]

此版本的 WebView2 SDK 要求Microsoft Edge版本 78.0.270.0 或更高版本。

### <a name="changes"></a>更改

*   添加了 `DocumentTitleChanged` 事件以指示文档标题 ([问题 27][GithubMicrosoftedgeWebviewfeedbackIssue27]) 。

*   添加了 `GetWebView2BrowserVersionInfo` API ([Issue #18][GithubMicrosoftedgeWebviewfeedbackIssue18]) 。
...
*   更新 `CreateWebView2EnvironmentWithDetails` 了 函数以删除 `releaseChannelPreference` 。  有关 函数详细信息 `CreateWebView2EnvironmentWithDetails` ，请导航到 [CreateWebView2EnvironmentWithDetails][Webview2ReferenceWin32Webview2IdlViewWebview208355Createwebview2environmentwithdetails]。  仍然支持注册表和环境变量替代。  除非重写，否则使用默认通道首选项。
    ...
    WebView2 团队选择更稳定的渠道，以确保最终用户的行为最一致。  使用最新的 Canary 版本进行测试时，应在启动应用之前创建一个脚本，将环境变量 `WEBVIEW2_RELEASE_CHANNEL_PREFERENCE` `1` 设置为 。  有关详细信息，请导航到 [切换到预览频道以测试即将推出的 API 和功能][SwitchToPreviewChannel]。

*   使用 `CreateWebView2EnvironmentWithDetails` 逻辑更新了函数，以选择 `userDataFolder` 未指定时。  有关 函数详细信息 `CreateWebView2EnvironmentWithDetails` ，请导航到 [CreateWebView2EnvironmentWithDetails][Webview2ReferenceWin32Webview2IdlViewWebview208355Createwebview2environmentwithdetails]。  如果之前使用了默认位置，则切换到新 SDK 时，默认设置 (设置为主机代码目录中的新位置，) 也会重置 `userDataFolder` `userDataFolder` 状态。  如果主机进程没有写入指定目录的权限，函数 `CreateWebView2EnvironmentWithDetails` 可能会失败。  你可以将数据从旧目录复制到 `user data folder` 新目录。


<!-- ====================================================================== -->
## <a name="08230"></a>0.8.230

[NuGet WebView2 SDK 0.8.230 的程序包][NuGetGallery0.8.230]

此版本的 WebView2 SDK 要求Microsoft Edge版本 77.0.230.0 或更高版本。

### <a name="changes"></a>更改

*   添加了 `Stop` API 以停止所有导航和挂起的资源提取 (问题 [28][GithubMicrosoftedgeWebviewfeedbackIssue28]) 。
*   向 `.tlb` 文件包NuGet问题 ([#22 文件) 。][GithubMicrosoftedgeWebviewfeedbackIssue22]
*   向 NuGet Issue [#32][GithubMicrosoftedgeWebviewfeedbackIssue32] (中的安装程序列表添加了 .NET) 。


<!-- ====================================================================== -->
## <a name="08190"></a>0.8.190

[NuGet WebView2 SDK 0.8.190 的程序包][NuGetGallery0.8.190]

此版本的 WebView2 SDK 要求Microsoft Edge版本 77.0.190.0 或更高版本。

*   已 `get_AreDevToolsEnabled` / `put_AreDevToolsEnabled` 添加到控制用户能否打开 DevTools (问题[#16][GithubMicrosoftedgeWebviewfeedbackIssue16]) 。
*   已 `get_IsStatusBarEnabled` / `put_IsStatusBarEnabled` 添加到控件中，以控制状态栏是否 ([问题 #19) 。][GithubMicrosoftedgeWebviewfeedbackIssue19]
...
*   在 64 位计算机上添加了 32 位 WebView ([Issue #13][GithubMicrosoftedgeWebviewfeedbackIssue13]) 。
*   向 SDK ([Issue #14][GithubMicrosoftedgeWebviewfeedbackIssue14]) 添加了 WebView IDL。
*   添加了 lib 以支持 `IID\_\*` 接口 ID 对象 (Issue [#12][GithubMicrosoftedgeWebviewfeedbackIssue12]) 。
...
[NuGet WebView2 SDK 0.8.149 的程序包][NuGetGallery0.8.149]

此版本的 WebView2 SDK 要求Microsoft Edge版本 76.0.149.0 或更高版本。
...
*  [联系 Microsoft Edge WebView2 团队][Contact]


<!-- ====================================================================== -->
<!-- links -->
[Contact]: contact.md "联系 Microsoft Edge WebView2 团队|Microsoft Edge开发人员文档"
[SwitchToPreviewChannel]: how-to/set-preview-channel.md "切换到预览频道以测试即将推出的 API 和功能|Microsoft Edge开发人员文档"
[Webview2ConceptsDistribution]: ./concepts/distribution.md "分发 WebView2 应用和 WebView2 运行时|Microsoft Docs"
[Webview2ConceptsDistributionUnderstandRuntimeInstaller]: ./concepts/distribution.md#understanding-the-webview2-runtime "了解 WebView2 运行时 - 分发 WebView2 应用和 WebView2 运行时|Microsoft Docs"
...
[Webview2ConceptsVersioningMatchingWebview2RuntimeVersions]: ./concepts/versioning.md#matching-the-runtime-version-with-the-sdk-version "将运行时版本与 SDK 版本匹配 - 了解 WebView2 SDK |Microsoft Docs"
...
[Webview2HowToDebug]: ./how-to/debug.md "如何使用 WebView2 控件进行开发时如何|Microsoft Docs"
<!-- external links -->
...
[GithubMicrosoftedgeWebviewfeedbackIssue616]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/616 "MicrosoftEdge/WebViewFeedback 问题 616 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1097]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1097 "MicrosoftEdge/WebViewFeedback 问题 1097 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1092]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1092 "MicrosoftEdge/WebViewFeedback 问题 1092 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1462]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1462 "MicrosoftEdge/WebViewFeedback 问题 1462 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1348]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1348 "MicrosoftEdge/WebViewFeedback 问题 1348 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1428]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1428 "MicrosoftEdge/WebViewFeedback 问题 1428 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1016]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1016 "MicrosoftEdge/WebViewFeedback 问题 1016 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue634]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/634 "MicrosoftEdge/WebViewFeedback 问题 634 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue935]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/935 "MicrosoftEdge/WebViewFeedback 问题 935 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue637]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/637 "MicrosoftEdge/WebViewFeedback 问题 637 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1142]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1142 "MicrosoftEdge/WebViewFeedback 问题 1142 的反馈存储库"

[GithubMicrosoftedgeWebviewfeedbackIssue1605]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1605 "MicrosoftEdge/WebViewFeedback 问题 1605 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1678]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1678 "MicrosoftEdge/WebViewFeedback 问题 1678 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1566]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1566 "MicrosoftEdge/WebViewFeedback 问题 1566 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1666]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1666 "MicrosoftEdge/WebViewFeedback 问题 1666 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1144]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1144 "MicrosoftEdge/WebViewFeedback 问题 1144 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue411]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/411 "MicrosoftEdge/WebViewFeedback 问题 411 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue238]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/238 "MicrosoftEdge/WebViewFeedback 问题 238 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1077]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1077 "MicrosoftEdge/WebViewFeedback 问题 1077 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1723]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1723 "MicrosoftEdge/WebViewFeedback 问题 1723 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1061]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1061 "MicrosoftEdge/WebViewFeedback 问题 1061 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue730]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/730 "MicrosoftEdge/WebViewFeedback 问题 730 的反馈存储库"
[GithubMicrosoftedgeWebviewfeedbackIssue1548]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1548 "MicrosoftEdge/WebViewFeedback 问题 1548 的反馈存储库"
[WebView2UDFAnnouncement]: https://github.com/MicrosoftEdge/WebViewFeedback/issues/1410 "MicrosoftEdge/WebViewFeedback 问题 1410 的反馈存储库"

[MicrosoftDevblogDotnetAnnouncingGeneralAvailabilityForMicrosoftEdgeWebview2ForNetFixedDistributionMethod]: https://devblogs.microsoft.com/dotnet/announcing-general-availability-for-microsoft-edge-webview2-for-net-and-fixed-distribution-method "宣布正式发布适用于 .NET Microsoft Edge .NET 的 WebView2 和固定分发方法| .NET 博客"
...
[NuGetGallery1.0.902.49]: https://www.nuget.org/packages/Microsoft.Web.WebView2/1.0.902.49 "NuGet库|Microsoft.Web.WebView2 v1.0.902.49"
[NuGetGallery1.0.955-prerelease]: https://www.nuget.org/packages/Microsoft.Web.WebView2/1.0.955-prerelease "NuGet库|Microsoft.Web.WebView2 v1.0.955 预发行"
[NuGetGallery1.0.961.33]: https://www.nuget.org/packages/Microsoft.Web.WebView2/1.0.961.33 "NuGet库|Microsoft.Web.WebView2 v1.0.961.33"
[NuGetGallery1.0.992.28]: https://www.nuget.org/packages/Microsoft.Web.WebView2/1.0.992.28 "NuGet库|Microsoft.Web.WebView2 v1.0.992.28"
[NuGetGallery1.0.1010-prerelease]: https://www.nuget.org/packages/Microsoft.Web.WebView2/1.0.1010-prerelease "NuGet库|Microsoft.Web.WebView2 v1.0.1010-prerelease"
[NuGetGallery1.0.1018-prerelease]: https://www.nuget.org/packages/Microsoft.Web.WebView2/1.0.1018-prerelease "NuGet库|Microsoft.Web.WebView2 v1.0.1018-prerelease"
<!-- end of NuGet links -->
...
[Webview2ReferenceWin32Icorewebview2setting2ViewWebview21086435GetUseragent]: /microsoft-edge/webview2/reference/win32/icorewebview2settings2?view=webview2-1.0.864.35&preserve-view=true#get_useragent "get_UserAgent - 接口 ICoreWebView2Settings2 |Microsoft Docs"
...
[Webview2ReferenceWin32Icorewebview2experimental4ViewWebview210955PrereleaseOpentaskmanagerwindow]: /microsoft-edge/webview2/reference/win32/icorewebview2experimental4?view=webview2-1.0.955-prerelease&preserve-view=true#opentaskmanagerwindow "OpenTaskManagerWindow - 接口 ICoreWebView2Experimental4 |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2experimentalnewwindowrequestedeventargsViewWebview210955PrereleaseGetName]: /microsoft-edge/webview2/reference/win32/icorewebview2experimentalnewwindowrequestedeventargs?view=webview2-1.0.955-prerelease&preserve-view=true#get_name "get_Name - 接口 ICoreWebView2ExperimentalNewWindowRequestedEventArgs |Microsoft Docs"

[Webview2ReferenceWin32Icorewebview25ViewWebview210955PrereleaseAddClientcertificaterequested]: /microsoft-edge/webview2/reference/win32/icorewebview2_5?view=webview2-1.0.955-prerelease&preserve-view=true#add_clientcertificaterequested "add_ClientCertificateRequested - 接口ICoreWebView2_5 |Microsoft Docs"

[Webview2ReferenceWin32Icorewebview24ViewWebview21090249AddFramecreated]: /microsoft-edge/webview2/reference/win32/icorewebview2_4?view=webview2-1.0.902.49&preserve-view=true#add_framecreated "add_FrameCreated - 接口ICoreWebView2_4 |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2settings4ViewWebview21090249GetIsgeneralautofillenabled]: /microsoft-edge/webview2/reference/win32/icorewebview2settings4?view=webview2-1.0.902.49&preserve-view=true#get_isgeneralautofillenabled "get_IsGeneralAutofillEnabled - 接口 ICoreWebView2Settings4 |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2settings5ViewWebview21090249GetIspinchzoomenabled]: /microsoft-edge/webview2/reference/win32/icorewebview2settings5?view=webview2-1.0.902.49&preserve-view=true#get_ispinchzoomenabled "get_IsPinchZoomEnabled - 接口 ICoreWebView2Settings5 |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2experimentalsettings6ViewWebview210955PrereleaseGetHiddenpdftoolbaritems]: /microsoft-edge/webview2/reference/win32/icorewebview2experimentalsettings6?view=webview2-1.0.955-prerelease&preserve-view=true#get_hiddenpdftoolbaritems "get_HiddenPdfToolbarItems - 接口 ICoreWebView2ExperimentalSettings6 |Microsoft Docs"
...
[Webview2ReferenceWin32Icorewebview2experimentalenvironment5ViewWebview210999PrereleaseGetUserdatafolder]: /microsoft-edge/webview2/reference/win32/icorewebview2experimentalenvironment5?view=webview2-1.0.1010-prerelease&preserve-view=true#get_userdatafolder "get_UserDataFolder - 接口 ICoreWebView2ExperimentalEnvironment5 |Microsoft Docs"
...
[Webview2ReferenceWin32Icorewebview2experimentalframeViewWebview210999PrereleaseAddNavigationstarting]: /microsoft-edge/webview2/reference/win32/icorewebview2experimentalframe?view=webview2-1.0.1010-prerelease&preserve-view=true#add_navigationstarting "add_NavigationStarting - 接口 ICoreWebView2ExperimentalFrame |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2experimentalframeViewWebview210999PrereleaseRemoveNavigationstarting]: /microsoft-edge/webview2/reference/win32/icorewebview2experimentalframe?view=webview2-1.0.1010-prerelease&preserve-view=true#remove_navigationstarting "remove_NavigationStarting - 接口 ICoreWebView2ExperimentalFrame |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2experimentalframeViewWebview210999PrereleaseAddContentloading]: /microsoft-edge/webview2/reference/win32/icorewebview2experimentalframe?view=webview2-1.0.1010-prerelease&preserve-view=true#add_contentloading "add_ContentLoading - 接口 ICoreWebView2ExperimentalFrame |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2experimentalframeViewWebview210999PrereleaseRemoveContentloading]: /microsoft-edge/webview2/reference/win32/icorewebview2experimentalframe?view=webview2-1.0.1010-prerelease&preserve-view=true#remove_contentloading "remove_ContentLoading - 接口 ICoreWebView2ExperimentalFrame |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2experimentalframeViewWebview210999PrereleaseAddNavigationcompleted]: /microsoft-edge/webview2/reference/win32/icorewebview2experimentalframe?view=webview2-1.0.1010-prerelease&preserve-view=true#add_navigationcompleted "add_NavigationCompleted - 接口 ICoreWebView2ExperimentalFrame |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2experimentalframeViewWebview210999PrereleaseRemoveNavigationcompleted]: /microsoft-edge/webview2/reference/win32/icorewebview2experimentalframe?view=webview2-1.0.1010-prerelease&preserve-view=true#remove_navigationcompleted "remove_NavigationCompleted - 接口 ICoreWebView2ExperimentalFrame |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2experimentalframeViewWebview210999PrereleaseAddDomcontentloaded]: /microsoft-edge/webview2/reference/win32/icorewebview2experimentalframe?view=webview2-1.0.1010-prerelease&preserve-view=true#add_domcontentloaded "add_DOMContentLoaded - 接口 ICoreWebView2ExperimentalFrame |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2experimentalframeViewWebview210999PrereleaseRemoveDomcontentloaded]: /microsoft-edge/webview2/reference/win32/icorewebview2experimentalframe?view=webview2-1.0.1010-prerelease&preserve-view=true#remove_domcontentloaded "remove_DOMContentLoaded - 接口 ICoreWebView2ExperimentalFrame |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2experimentalframeViewWebview210999PrereleaseExecutescript]: /microsoft-edge/webview2/reference/win32/icorewebview2experimentalframe?view=webview2-1.0.1010-prerelease&preserve-view=true#executescript "ExecuteScript - 接口 ICoreWebView2ExperimentalFrame |Microsoft Docs"
...
[Webview2ReferenceWin32Icorewebview26ViewWebview210999Prerelease]: /microsoft-edge/webview2/reference/win32/icorewebview2_6?view=webview2-1.0.1010-prerelease&preserve-view=true "接口ICoreWebView2_6 |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview26ViewWebview210999PrereleaseOpentaskmanagerwindow]: /microsoft-edge/webview2/reference/win32/icorewebview2_6?view=webview2-1.0.1010-prerelease&preserve-view=true#opentaskmanagerwindow "OpenTaskManagerWindow - 接口ICoreWebView2_6 |Microsoft Docs"
...
[Webview2ReferenceWin32Icorewebview2environment5ViewWebview210999PrereleaseAddBrowserprocessexited]: /microsoft-edge/webview2/reference/win32/icorewebview2environment5?view=webview2-1.0.1010-prerelease&preserve-view=true#add_browserprocessexited "add_BrowserProcessExited - 接口 ICoreWebView2Environment5 |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2environment5ViewWebview210999PrereleaseRemoveBrowserprocessexited]: /microsoft-edge/webview2/reference/win32/icorewebview2environment5?view=webview2-1.0.1010-prerelease&preserve-view=true#remove_browserprocessexited "remove_BrowserProcessExited - 接口 ICoreWebView2Environment5 |Microsoft Docs"

[Webview2ReferenceWin32Icorewebview25ViewWebview21096132]: /microsoft-edge/webview2/reference/win32/icorewebview2_5?view=webview2-1.0.961.33&preserve-view=true "接口ICoreWebView2_5 |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview25ViewWebview21096132AddClientcertificaterequested]: /microsoft-edge/webview2/reference/win32/icorewebview2_5?view=webview2-1.0.961.33&preserve-view=true#add_clientcertificaterequested "add_ClientCertificateRequested - 接口ICoreWebView2_5 |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview25ViewWebview21096132RemoveClientcertificaterequested]: /microsoft-edge/webview2/reference/win32/icorewebview2_5?view=webview2-1.0.961.33&preserve-view=true#remove_clientcertificaterequested "remove_ClientCertificateRequested - 接口ICoreWebView2_5 |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificateViewWebview21096132]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificate?view=webview2-1.0.961.33&preserve-view=true "interface ICoreWebView2ClientCertificate |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificateViewWebview21096132GetSubject]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificate?view=webview2-1.0.961.33&preserve-view=true#get_subject "get_Subject - 接口 ICoreWebView2ClientCertificate |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificateViewWebview21096132GetIssuer]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificate?view=webview2-1.0.961.33&preserve-view=true#get_issuer "get_Issuer - 接口 ICoreWebView2ClientCertificate |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificateViewWebview21096132GetValidfrom]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificate?view=webview2-1.0.961.33&preserve-view=true#get_validfrom "get_ValidFrom - 接口 ICoreWebView2ClientCertificate |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificateViewWebview21096132GetValidto]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificate?view=webview2-1.0.961.33&preserve-view=true#get_validto "get_ValidTo - 接口 ICoreWebView2ClientCertificate |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificateViewWebview21096132GetDerencodedserialnumber]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificate?view=webview2-1.0.961.33&preserve-view=true#get_derencodedserialnumber "get_DerEncodedSerialNumber - 接口 ICoreWebView2ClientCertificate |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificateViewWebview21096132GetDisplayname]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificate?view=webview2-1.0.961.33&preserve-view=true#get_displayname "get_DisplayName - 接口 ICoreWebView2ClientCertificate |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificateViewWebview21096132Topemencoding]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificate?view=webview2-1.0.961.33&preserve-view=true#topemencoding "ToPemEncoding - 接口 ICoreWebView2ClientCertificate |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificateViewWebview21096132GetPemencodedissuercertificatechain]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificate?view=webview2-1.0.961.33&preserve-view=true#get_pemencodedissuercertificatechain "get_PemEncodedIssuerCertificateChain - 接口 ICoreWebView2ClientCertificate |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificateViewWebview21096132GetKind]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificate?view=webview2-1.0.961.33&preserve-view=true#get_kind "get_Kind - 接口 ICoreWebView2ClientCertificate |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificatecollectionViewWebview21096132]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificatecollection?view=webview2-1.0.961.33&preserve-view=true "interface ICoreWebView2ClientCertificateCollection |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificatecollectionViewWebview21096132GetCount]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificatecollection?view=webview2-1.0.961.33&preserve-view=true#get_count "get_Count - 接口 ICoreWebView2ClientCertificateCollection |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2clientcertificatecollectionViewWebview21096132Getvalueatindex]: /microsoft-edge/webview2/reference/win32/icorewebview2clientcertificatecollection?view=webview2-1.0.961.33&preserve-view=true#getvalueatindex "GetValueAtIndex - 接口 ICoreWebView2ClientCertificateCollection |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2stringcollectionViewWebview21096132]: /microsoft-edge/webview2/reference/win32/icorewebview2stringcollection?view=webview2-1.0.961.33&preserve-view=true "interface ICoreWebView2StringCollection |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2stringcollectionViewWebview21096132GetCount]: /microsoft-edge/webview2/reference/win32/icorewebview2stringcollection?view=webview2-1.0.961.33&preserve-view=true#get_count "get_Count - 接口 ICoreWebView2StringCollection |Microsoft Docs"
[Webview2ReferenceWin32Icorewebview2stringcollectionViewWebview21096132Getvalueatindex]: /microsoft-edge/webview2/reference/win32/icorewebview2stringcollection?view=webview2-1.0.961.33&preserve-view=true#getvalueatindex "GetValueAtIndex - 接口 ICoreWebView2StringCollection |Microsoft Docs"
...
[OpenTaskManagerWindowAPI]: /microsoft-edge/webview2/reference/win32/icorewebview2_6?view=webview2-1.0.992.28&preserve-view=true#summary "接口ICoreWebView2_6 |Microsoft Docs"

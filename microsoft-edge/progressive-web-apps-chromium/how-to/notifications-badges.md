...
`clearAppBadge`还会返回可用于处理潜在错误的 Promise。

另一种清除锁屏提醒的方式是 `setAppBadge` 再次调用，但传递 `0` 为值，这次：
...
上述代码显示纯文本通知消息，但您也可以通过添加其他 和 属性来 `body` 自定义 `icon` 邮件：
...
`showNotification`函数支持与上一示例 `Notification` 中使用的构造函数相同的参数。  `showNotification`函数还支持 `actions` 属性，如下一节所述。

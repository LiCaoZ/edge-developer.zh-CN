...
如果你正在查找对函数（如 ）的 API 引用， `console.log()` 请导航到"[控制台 API 参考"。][DevToolsConsoleApi]  有关 `monitorEvents()` 等函数的参考，请导航到[控制台实用工具 API 参考][DevToolsConsoleUtilities]。  

...
使用 **"** 网络"工具确保资源已如期下载或上载。  检查单个资源的属性，如 HTTP 标头、内容或大小。
...
1. 若要打开 DevTools，请右键单击该网页，然后选择"检查 **"**。  或者，按 `Ctrl`++`Shift``J` (Windows、Linux) 或 (`J` `Command`+`Option`+macOS) 。  将打开 DevTools。

1. 在 DevTools 中，在主工具栏上，选择"控制台 **"** 选项卡。 如果该选项卡不可见，****![请单击"更多选项卡" (更多选项卡"图标](../media/more-tabs-icon-light-theme.png)。) 按钮：
...
**网络**工具最初为空。  DevTools 仅在打开网络活动后记录网络活动，并且自打开 DevTools 后未发生任何网络活动。
...
1. 长按"**重新加载**![ (重新加载****](../media/refresh-icon.msft.png)"。)  (或右键单击"刷新) ，然后选择"空缓存**和硬刷新"**：

   :::image type="content" source="../media/network-glitch-empty-cache-and-hard-reset.msft.png" alt-text="空缓存和硬刷新。" lightbox="../media/network-glitch-empty-cache-and-hard-reset.msft.png":::

在重复访问时，浏览器通常会从[缓存](https://developer.mozilla.org/docs/Web/HTTP/Caching)中提供一些文件，从而加快页面加载速度。  **空缓存和硬刷新** 会强制浏览器访问所有资源的网络。  使用它来显示第一次访问者如何体验页面加载。

空 **缓存和硬刷新** 工作流仅在 DevTools 打开时可用。
...
1. 使用空缓存和硬刷新工作流 **再次刷新** 页面。  如果需要 [有关操作方式的](#simulate-a-slower-network-connection) 提醒，请参阅模拟上述较慢的连接。
...
1. 选择 **响应** 面板。  将显示 HTML 源代码：
   ...
   **** 提示：![](../media/format-icon.msft.png)缩小文件时，选择" (格式"。选择"响应"面板底部的"**) "按钮**，以重新设置文件内容的格式，提高可读性。
...
1. 选择 **"搜索** (![搜索"。](../media/search-icon.msft.png)) 。  "搜索"窗格在网络日志的左侧打开：
...
1. 键入 并 `no-cache` 按 `Enter`。  "搜索"窗格列出了 `no-cache` 它在资源标头或内容中查找到的所有实例：
...
1. 单击结果以查看找到结果的资源。  如果要查看资源的详细信息，请选择直接转到它的结果。  例如，如果在标头中发现查询，则 **标题面板** 将打开。   如果在内容中发现了查询，则" **响应"面板** 将打开：
...
1. 键入 `png` 到“**筛选器**”文本框。  只显示包含文本 `png` 的文件。  在这种情况下，仅与筛选器匹配的文件是 PNG 图像：

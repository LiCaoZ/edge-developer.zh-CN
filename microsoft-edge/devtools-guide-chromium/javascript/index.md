...
ms.date: 05/04/2021
ms.openlocfilehash: 5886b0f9a7a177f9b7180b5b5bcf63f8288d1256
ms.sourcegitcommit: c63325d520191ce4b4e707fb680c84afce4eab54
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 02/02/2022
ms.locfileid: "12338813"
---
...
1. 打开新 [选项卡或窗口中](https://microsoftedge.github.io/Demos/devtools-js-get-started/) 的调试演示页面。  为此，请右`Ctrl`键单击或长按 (Windows、Linux) 或 `Command` (macOS) ，然后单击链接。
...
1.  若要在 **** DevTools `Control``J``Shift`++中打开控制台工具，请选择 (Windows、Linux) 或`J` `Command`+`Option`+ (macOS) 。
...
如果你后退一步并思考应用的工作原理，你可以有根据地猜测错误总和 (`5 + 1 = 51`) 是在与"添加数字 1"和"数字 **2**"`click`按钮关联的事件侦听器中计算的。  因此，可能想在 `click` 侦听器运行期间暂停代码。  **事件侦听器断点**可以完全实现此要求：
...
1.  返回到演示，再次选择“**添加数字 1 和"数字 2**”。  开发工具暂停演示并在“**源**”具中突出显示一行代码。  开发工具应该暂停在 `get-started.js` 中的第 16 行。
    ...
    如果您在其他代码行上****![暂停，请选择"恢复脚本执行" ("恢复](../media/resume-script-run-icon.msft.png)脚本执行"。) ，直到您暂停到正确的行。
...
1.  在 DevTools **** **** ![](../media/step-into-icon.msft.png) `updateLabel()` 的"源"工具上，选择"单步执行下一个函数调用 (""单步执行下一个函数调用"。) 可单步执行函数的运行时，一次一行。
...
监视 **窗格** 允许您 `sum` 监视变量的值，例如 (或) 表达式 (如) `typeof sum` 。  可以将任何有效的 JavaScript 表达式存储在监视表达式中。

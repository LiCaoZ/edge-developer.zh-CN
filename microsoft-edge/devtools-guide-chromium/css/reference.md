...
*  在 DevTools 中，单击"选择**元素 (** ![](../media/select-an-element-icon.msft.png) 选择元素"。`Shift``C`+`Control`+) 或按 (Windows、Linux) `C` `Command`+`Shift`+或 (macOS) ，然后单击视口中的元素。
...
打开颜色选取**器时******![，取色器 (取](../media/eyedropper-icon.msft.png)色器。) 打开。

...
若要访问 3D 视图，请导航到 `edge://flags` 并确保开发人员 **工具实验** 标记设置为 **已启用**。  重新启动Microsoft Edge并打开 DevTools。  在 DevTools 中选择或打开设置实验"部分，然后打开 `F1` ****  >  ****"**启用 3D 视图"** 复选框。  现在，选择 `Ctrl`  +  `Shift`  +  `P` ，在**3D 视图中键入 ，** 然后选择**显示 3D 视图**。  

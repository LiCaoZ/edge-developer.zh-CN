...
ms.date: 04/23/2021
ms.openlocfilehash: 9014f688bbaa31499ef3838782157b2171518e6d
ms.sourcegitcommit: 6fa0ef440a4e4565a2055dc2742d5d1bf8744939
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 12/21/2021
ms.locfileid: "12287239"
---
...
*   将来自你的 Windows内容的屏幕广播到开发计算机上 DevTools 实例。


<!-- ====================================================================== -->
## <a name="step-1-set-up-the-host-debuggee-machine"></a>步骤 1：设置 (调试程序计算机) 

主机或调试程序计算机是Windows 10调试的设备或更高版本的设备。  这可能是一个远程设备，你很难从物理上访问，或者可能没有键盘和鼠标外设，因此很难与该设备上的 Microsoft Edge DevTools 交互。  若要在调试 () 主机，需要完成以下操作。
...
1.  如果尚未在要调试Windows 10或更高版本的设备上，从Microsoft Edge[安装。](https://www.microsoft.com/edge)
...
> [!NOTE]
> 如果你**使用**Microsoft Edge ([Windows 10](https://www.microsoft.com/store/apps/9P6CMFV44ZLT)或更高版本 1809 或更早版本) Beta 版远程工具的"获取"按钮可能处于禁用状态。  若要在调试 (中) 主机，必须运行 Windows 10版本 1903 或更高版本。  更新调试 (调试) 计算机以获取 Beta [Microsoft Edge (的](https://www.microsoft.com/store/apps/9P6CMFV44ZLT)远程) 。

:::image type="content" source="../media/remote-debugging-windows-media-remote-tools-in-store.msft.png" alt-text="Microsoft Store中Microsoft Edge (Beta) 工具。" lightbox="../media/remote-debugging-windows-media-remote-tools-in-store.msft.png":::
...
如果你的网络列为"公用"，请转到设置网络********&  >  **Internet**  >  **WI-Fi"，** 单击网络，将"网络**配置文件**"按钮切换为 **"专用"。**
...
> [!NOTE]
> 如果 **通过本地网络连接打开远程诊断。** 之前已打开，必须将其关闭并再次打开 **，Device Portal**可以使用 Microsoft Edge ([Beta ](https://www.microsoft.com/store/apps/9P6CMFV44ZLT)) 。  如果 **"For developers"** 部分未显示在设置**** 中 **，Device Portal**可能已经打开，因此请尝试Windows 10或更高版本的设备。
...
注意在设备下显示计算机 IP 地址和连接**端口：。**  下图中的 IP 地址为 `192.168.86.78` ，连接端口为 `50080` ：
...
你将在客户端调试器 (下一) 在设备上 [输入信息](#step-2-set-up-the-client-debugger-machine)。  在主机 (调试程序[ () ](../progressive-web-apps/index.md)计算机上打开要从客户端 (调试器) 进行调试的 Microsoft Edge 和渐进式 Web 应用) PA) 中的选项卡。
...
1.  若要将客户端 (调试) 计算机，Microsoft Edge，请从此页安装客户端。 [](https://www.microsoft.com/edge)
...
1.  如果为受调试 (的) 计算机设置身份验证，系统将提示你输入用户名和密码，以便客户端 (调试) 计算机**** 成功连接****。
...
1.  现在，在使用页面从 () 客户端 (调试器) 连接到受调试者计算机时，必须使用 `edge://inspect` 其他 `connection port` 值。  默认情况下，对于桌面Windows，Device Portal 使用 作为 `50080` 的 `connection port` `http` 。  对于 `https` ，Device Portal 使用 `50043` ，因此请遵循以下模式：https:// `IP address` `50043` `edge://inspect` ：。  [阅读有关 Device Portal 使用的默认端口的更多信息](/windows/uwp/debug-test-perf/device-portal#setup)。

> [!NOTE]
> 的默认端口为 ，默认端口为 ，但并不总是这样，因为桌面版 Device Portal 声明临时范围 `http` `50080` `https` `50043` (\>50，000) 中的端口，以防止与设备上现有的端口声明发生冲突。  若要了解更多信息，请参阅适用于桌面[设置](/windows/uwp/debug-test-perf/device-portal-desktop#registry-based-configuration-for-device-portal)Device Portal 的移植Windows部分。

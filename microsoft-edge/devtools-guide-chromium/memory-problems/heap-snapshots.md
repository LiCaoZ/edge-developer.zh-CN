...
选择 **"清除所有配置文件** "图标，从 DevTools (删除快照与呈现器进程关联的任何内存\) 。  
...
> [!NOTE]
> 试用此散 [点对象示例，][GlitchDevtoolsMemoryExample03] 然后使用堆探查器进行配置文件。  将显示一个 \ (对象\) 分配。  

## <a name="view-snapshots"></a>查看快照  

从不同的角度查看不同任务的快照。  

**摘要视图** 显示按构造函数名称分组的对象。  使用它来根据按构造函数名称 (分组的类型来搜寻对象 \ (和内存 use\) 。  它尤其有助于跟踪 **DOM 泄露**。
...
堆探查器能够反映浏览器本机对象 \ (DOM 节点、CSS 规则\) JavaScript 对象之间的双向依赖关系。

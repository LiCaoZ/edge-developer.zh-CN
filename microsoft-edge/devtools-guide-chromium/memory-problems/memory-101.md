...
使用 DevTools \ (内存面板时 (在**Memory**\) 下发现的内存问题的工具，你可能会发现自己正在查看一些不同的信息列。 [][DevtoolsMemoryProblemsHeapSnapshots]  两个突出的"浅 **表** 大小"和 **"保留大小**"，但这表示什么？  

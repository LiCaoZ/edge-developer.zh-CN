---
title: 支持Microsoft Edge扩展的 API
description: 生成Microsoft Edge扩展时要使用的受支持 API 列表。
author: MSEdgeTeam
ms.author: msedgedevrel
ms.topic: conceptual
ms.prod: microsoft-edge
ms.date: 01/07/2021
ms.openlocfilehash: 3b155aed1446273009d523e35fe66ee023e455dd
ms.sourcegitcommit: 855d6986920b5d231e68a453bc8e5b31180f3026
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 05/06/2022
ms.locfileid: "12506776"
---
# <a name="supported-apis-for-microsoft-edge-extensions"></a>支持Microsoft Edge扩展的 API

为Microsoft Edge浏览器生成扩展时，可以使用以下 API。
...
| [browserAction](https://developer.chrome.com/extensions/browserAction) | 使用浏览器操作将图标放在Microsoft Edge的工具栏上。 还可以使用浏览器操作来添加工具提示、锁屏提醒或弹出窗口。 |
...
| [contentSettings](https://developer.chrome.com/extensions/contentSettings) | 通常，内容设置允许你自定义每个站点上（而不是全局）Microsoft Edge的行为。 更改用于控制网站是否可以使用 Cookie、JavaScript 和插件等功能的设置。 |
| [contextMenus](https://developer.chrome.com/extensions/contextMenus) | 将项目添加到Microsoft Edge中的上下文菜单。 菜单项可能适用于不同的对象，例如图像、超链接和页面。 |
...
| [extensionTypes](https://developer.chrome.com/extensions/extensionTypes) | 包含Microsoft Edge扩展的类型声明。 |
| [fontSettings](https://developer.chrome.com/extensions/fontSettings) | 在Microsoft Edge中管理字体设置。 |
...
| [omnibox](https://developer.chrome.com/extensions/omnibox) | 在Microsoft Edge地址栏（也称为全框）中注册关键字。 |
| [pageAction](https://developer.chrome.com/extensions/pageAction) | 将图标添加到地址栏右侧的Microsoft Edge工具栏。 页面操作是可以在当前页面上执行的操作，不适用于所有页面。 当处于非活动状态时，页面操作将显示灰显。 |
...
| [隐私](https://developer.chrome.com/extensions/privacy) | 控制Microsoft Edge中影响用户隐私的功能。 此 API 取决于`EdgeSetting`获取和设置Microsoft Edge的配置的原型`types`。 |
| [代理](https://developer.chrome.com/extensions/proxy) | 管理Microsoft Edge的代理设置。 此 API 依赖于 `EdgeSetting` API 的`types`原型来获取和设置Microsoft Edge的代理配置。 |
...
| [类型](https://developer.chrome.com/extensions/types) | Microsoft Edge的类型声明。 |
...
*  用户必须使用 MICROSOFT 帐户登录到Microsoft Edge (MSA) 或Azure Active Directory帐户才能使用`chrome.identity.getProfileUserInfo`。 如果用户使用本地 Active Directory帐户登录到Microsoft Edge，则该 API 将返回`null`电子邮件和 ID 值。

   MSA)  (Microsoft 帐户是一个Outlook.com、Live.com 或 Hotmail.com 帐户。

*  Microsoft Edge不支持使用Chrome Web Store付款的扩展，因为它用于`identity.getAuthtoken`请求已登录用户的令牌。  这些令牌将发送到基于 REST 的许可 API。

...
将 `nativeMessaging` 权限添加到扩展 **manifest.js** 上的文件。  以下代码段是 上manifest.js** 的示例**。  
...
Microsoft Edge查询后 `HKEY_CURRENT_USER` 跟 的根键 `HKEY_LOCAL_MACHINE` 。  在这两个项中，首先搜索 32 位注册表，然后搜索 64 位注册表以标识本机消息传递主机。  注册表项指定本机消息传递主机清单的位置。  如果 Microsoft Edge注册表项没有主机清单的位置，Chromium 和 Chrome 注册表位置将用作回退选项。  如果Microsoft Edge在先前列出的任何位置找到注册表项，则它不会查询以下代码片段中列出的位置。  如果作为批处理脚本的一部分运行所创建的文件， `.reg` 请确保使用管理员命令提示符运行它。

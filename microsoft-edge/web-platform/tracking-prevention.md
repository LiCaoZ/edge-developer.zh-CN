...
ms.openlocfilehash: 5681f807720ba13dc310ec8112b2f7f42b1e3894
ms.sourcegitcommit: 09975d536fb4673442f2ac6629e1787f14f110e1
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 09/23/2021
ms.locfileid: "12035997"
---
...
在深入分析缓解之前，需要先定义"组织"或"组织"这一概念。  [Disconnect][|::ref3::|Main] 还维护一个称为 [entities.json][GitHubDisconnectMeTrackingProtectionEntitiesJson] 的列表，该列表定义由同一父组织/公司拥有的 URL 组。  组织中的跟踪防护功能Microsoft Edge组织关系缓解和组织参与缓解中使用此列表[](#org-relationship-mitigation)，以最大程度地减少因[](#org-engagement-mitigation)跟踪防护（影响跨组织请求）导致的兼容性问题的发生。

### <a name="org-relationship-mitigation"></a>组织关系缓解

多个热门网站同时维护网站和内容交付网络 \ (CDN\) 为这些网站提供静态资源和内容。  为了确保这些类型的方案不受跟踪防护的影响，Microsoft Edge 在网站向同一父组织拥有的其他网站提出第三方请求时，Microsoft Edge 会免除网站跟踪防护，如[Disconnect entities.json list][GitHubDisconnectMeTrackingProtectionEntitiesJson] (\) 中的定义。  这一点最好通过一个示例来说明。

> **示例：**
>
> 名为 Org1 的组织拥有域 和 `org1.test` `org1-cdn.test` ，如 [Disconnect entities.json 列表中的定义][GitHubDisconnectMeTrackingProtectionEntitiesJson]。  Imagine `org1-cdn.test` 分类为跟踪器并且通常会应用跟踪防护措施。  如果用户访问网站并且网站尝试从 加载资源，Microsoft Edge不会对请求执行任何强制操作，即使这不是第一方 `https://org1.test` `https://org1-cdn.test` `org1-cdn.test` URL。  但是，如果另一个不是 Org1 组织的一部分的 URL 尝试加载同一资源，则请求将受强制执行，因为它不是同一组织的一部分。
>
...
创建组织参与缓解是为了确保用户充分参与的组织所拥有的网站继续按预期在 Web 上工作，从而最大限度地降低通过跟踪防护引入的兼容性风险。  每当用户与给定[][ChromiumDesignDocsSiteEngagement]网站建立持续关系 \ (当前由网站参与分数 4.1 或更高\) 定义时，它就会利用网站参与来) 强制执行。  这一点最好通过一个示例来说明：
...
>
> 如果另一个网站 包含第三方内容 \ (则说明来自 Social Org 拥有的任何域的嵌入视频（来自 ) Social Org 拥有的任何域，这些域通常会通过跟踪防护实施进行限制）。只要用户的网站参与度分数（由 `https://content-embedder.example` Social Org 拥有的域）维持在阈值之上，该网站就免于跟踪防护实施。 `social-videos.example`
>
...
为了避免继续维护此列表，Microsoft 当前正在处理 存储[基本][GitHubMsExplainersStorageAccessApi]代码中的 Chromium API。  [存储 Access API][GitHubMsExplainersStorageAccessApi]为网站开发人员提供了直接从用户请求存储访问权限的方法，为用户提供了更透明的隐私设置如何影响其浏览体验的透明度，并给予网站开发人员控件快速直观地取消阻止。
...
在 Microsoft Edge 79 中，默认行为是在 InPrivate 中应用 Strict 模式保护。  在 Microsoft Edge 80 中，此行为已被开关所取代，该开关允许用户在浏览 InPrivate 时决定是应用严格模式保护还是保留其 `edge://settings/privacy` 常规设置。
...
[GitHubMsExplainersStorageAccessApi]: https://github.com/MicrosoftEdge/MSEdgeExplainers/blob/master/StorageAccessAPI/explainer.md "存储Access API 解释器 - MSEdgeExplainers/StorageAccessAPI |GitHub"

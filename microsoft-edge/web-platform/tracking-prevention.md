...
title: '跟踪防护Microsoft Edge (Chromium) '
author: MSEdgeTeam
...
ms.locfileid: "11976600"
---
# <a name="tracking-prevention-in-microsoft-edge-chromium"></a>跟踪防护Microsoft Edge (Chromium)   
...
如果这些主机名中的任一主机名与断开连接列表上的主机[][|::ref2::|Main][][GitHubDisconnectMeTrackingProtection]名匹配，Microsoft Edge继续执行强制操作，以防止跟踪用户。  
...
> **示例：**
> 
> 名为 Org1 的组织拥有域 和 `org1.test` `org1-cdn.test` ，如 [Disconnect entities.json 列表中的定义][GitHubDisconnectMeTrackingProtectionEntitiesJson]。  Imagine分类为跟踪器 `org1-cdn.test` 并且通常会应用跟踪防护措施。  如果用户访问网站并且网站尝试从 加载资源，Microsoft Edge将不会对请求执行任何强制操作，即使这不是第一方 `https://org1.test` `https://org1-cdn.test` `org1-cdn.test` URL。  但是，如果另一个不是 Org1 组织的一部分的 URL 尝试加载同一资源，则由于该请求不是同一组织的一部分，因此该请求将强制执行。  
> 
...
> 
> 如果另一个网站 包含第三方内容 \ (则说明来自 Social Org 所拥有的任何域的 ) 嵌入视频（来自 Social Org 拥有的任何域，该域通常会受到跟踪防护实施的限制）的嵌入视频，只要 Social Org 拥有域的用户网站参与分数维持在阈值之上，该网站就免于跟踪防护强制。 `https://content-embedder.example` `social-videos.example`
> 
...
根据 Microsoft 收到的最新用户反馈，Microsoft Edge 维护一小部分网站 \ (其中大多数网站位于断开连接内容类别\) 尽管已执行上述两项缓解措施，但由于跟踪防护，这些网站已中断。 此列表上的网站免于跟踪防护实施。  可以在磁盘上的以下位置 [找到](#determining-whetherhow-a-particular-url-is-classified) 该列表。  用户可以使用 中的"阻止" **选项覆盖其** 上的条目 `edge://settings/content/cookies` 。

为了避免继续维护此列表，Microsoft 当前正在存储[基本][GitHubMsExplainersStorageAccessApi]代码中的 Chromium Access API。  [存储 Access API][GitHubMsExplainersStorageAccessApi]为网站开发人员提供了直接从用户请求存储访问权限的方法，为用户提供了更透明的隐私设置如何影响其浏览体验的透明度，并给予网站开发人员控件快速直观地取消阻止。

实施[存储 Access API][GitHubMsExplainersStorageAccessApi]后，Microsoft 将弃用 CompatExceptions 列表，并联系受影响的站点，使其了解问题，并请求他们使用[存储 Access API。][GitHubMsExplainersStorageAccessApi]  
...
在 Microsoft Edge 79 中，默认行为是在 InPrivate 中应用 Strict 模式保护。  在 Microsoft Edge 80 中，此行为已被开关所取代，该开关允许用户在浏览 InPrivate 时决定是应用严格模式保护还是保留 `edge://settings/privacy` 其常规设置。  
